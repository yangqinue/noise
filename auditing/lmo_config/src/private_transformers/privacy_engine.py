...
from ...lmo import generate_lmo_noise
    ...
    def __init__(
        ...
        **unused_kwargs,
        ...
        Args:
            ...
            noise_multiplier: The extra multiplier for DP-SGD noise or the epsilon threshold for LMO.
        ...
        if lmo_filepath is not None:
            if lmo_filepath == "gaussian":
                self.lmo_params = False
            elif lmo_filepath == "lmo":
                from ...lmo_args import LMO_ARGS
                self.lmo_params = LMO_ARGS[float(noise_multiplier)]
        else:
            self.lmo_params = False
    ...
    def _create_noisy_clipped_gradient(self):
        ...
        for name, param in self.named_params:
            ...
            if self.noise_multiplier > 0 and self.max_grad_norm > 0:
                if self.lmo_params:
                    from ...lmo import DEFAULT_DISTRIBUTIONS
                    noise_lmo = generate_lmo_noise(self.lmo_params, DEFAULT_DISTRIBUTIONS, param.size())
                    
                    noise = torch.tensor(
                        noise_lmo.reshape(param.size()),
                        device=param.device,
                        dtype=param.dtype,
                    )
                else:
