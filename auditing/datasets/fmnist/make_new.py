...
print(trn_x.shape) # (60000, 28, 28)
print(trn_y.shape) # (60000,)
print(tst_x.shape) # (10000, 28, 28)
print(tst_y.shape) # (10000,)

np.random.seed(0)

n_features = 784

trn_inds = np.where(trn_y < 2)[0] # about 1/5 的量
tst_inds = np.where(tst_y < 2)[0] # about 1/5 的量
print(np.where(trn_y < 2))
...
for pois_ct in [1, 2, 4, 8, 16]:
    ...
    cp_x[-pois_ct:] = new_x
    print(np.sort(np.dot(cp_x, new_x))[-2*pois_ct:]) # 这行是什么意思？
    cp_x = cp_x.reshape((-1, 28, 28, 1))
    # cp_x是poison之后 替换pois_ct个数的训练数据x
    
    cp_y[-pois_ct:] = new_y
    cp_y2 = np.copy(trn_y)
    cp_y2[-pois_ct:] = (1-new_y)
    # cp_y是poison之后 替换pois_ct个数的训练数据的label y
    # cp_y2是poison之后 替换pois_ct个数的训练数据的label y2

    old_arrs = (trn_x.reshape((-1, 28, 28, 1)), trn_y), (cp_x, cp_y), (new_x, new_y), (tst_x.reshape((-1, 28, 28, 1)), tst_y)
    #（原始数据），（用下毒数据替换了pois_ct行后的新数据），（下毒数据），（测试数据）
